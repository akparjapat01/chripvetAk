...
import {mentorVerifyOTP, mentorVerifyEmail} from '../../redux/authaction';
import { black_1 } from '../../constants/colors';
import styles from './index.style';

const MentorOtp = props => {
  ...
  const verifyOtp = useSelector(state => state.mentorVerifyOtp);  
  ...
  const handleVerifyOtp = () => {
    ...
    dispatch(mentorVerifyOTP(email, allDigit));
  };
  ...
  null : 
  () => useSelector(state => state.mentorVerifyEmail);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (userType === 'MENTOR') {
        navigation.navigate('MentorCreateAccount', {email, userType});
      }
