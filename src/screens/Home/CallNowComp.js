...
import { agoraCallStart, deleteAppointment_Mentee } from '../../API/endpoints';
import Mute from 'react-native-vector-icons/Ionicons';
import { notificationSound } from '../../..';
import { show } from '../../utils/toast';

const CallNowComp = ({ list }) => {
  const data = list?.[0];
  ...
  const getChannel = async () => {
    ...
    formData.append('receiver_user_id', mentorId);
    formData.append('sender_user_id', menteeId);
  ...
  const handleDismiss = async () => {
    ...
    let res = await PostAPIRequest({ formData: formData, endpoint: deleteAppointment_Mentee, token: token });
    if(res?.status == 200){
        ...
        <TouchableOpacity
          ...
          <View style={styles.beautyFirstContainerBox}>
            <TouchableOpacity
              ...
              activeOpacity={0.8}>
              {data?.mentor_details[0]?.profile_pic ? (
                <>
                  <Image
                    source={{
                      uri: `${BASE_URL_IMAGE}${data?.mentor_details[0]?.profile_pic}`,
                    }}
            ...
            <View style={{ width: '68%', marginLeft: 10 }}>
              ...
              <View style={{ flexDirection: 'row' }}>
                <TouchableOpacity
                  ...
                  style={[
                    styles.callBox,
                    { backgroundColor: cobalt, marginTop: 10, width: 70,
                    height:28 },
                  ]}>
