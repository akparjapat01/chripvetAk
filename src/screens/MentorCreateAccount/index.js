...
import {mentorCreateAccount, mentorLogin} from '../../redux/authaction';
...
const MentorCreateAccount = props => {
  ...
  const createUserAccount = useSelector(state => state.mentorCreateAccount);
  ...
  const handleSubmit = () => {
    if (
      ...
      show('Please enter required field', "warning");
      return;
    }
    if (!name) {
    ...
    dispatch(mentorCreateAccount(formData));
  };
  ...
  useEffect(() => {
    const fetchSpecialityData = async () => {
      await axios
        .get(BASE_URL + '/admin/getAllSpecialist')
        .then(response => {
          setSpecialityList(response.data.data);
        })
        .catch(error => {
          console.log('Specialist', error.response);
        });
    };
    fetchSpecialityData();
  }, []);
  ...
  useEffect(() => {
    if (user && user.user && user.user.code === 200) {
      if (
        ...
        user.user.data.tokens.refresh.token !== undefined &&
        button
      ) {
  ...
  useEffect(() => {
    if (
      ...
      createUserAccount.success.data.message === 'New Mentor Created' &&
      button
    ) {
...
export default MentorCreateAccount;
